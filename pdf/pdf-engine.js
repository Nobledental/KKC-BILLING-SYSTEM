/* ============================================================================
   KRISHNA KIDNEY CENTRE — PDF ENGINE (Ceramic V4 • Final Production)
   ✔ 100% Offline
   ✔ Tamil footer (Option B)
   ✔ AutoTable A4 layout
   ✔ Corrected amount-in-words (uses inrToWords)
   ✔ Compatible with jsPDF UMD (window.jspdf.jsPDF)
============================================================================ */
...
async function exportPremiumPDF() {
  ...
  if (hospital.logo) {
    try {
      ...
      console.warn("Logo draw error:", err);
    }
  }
  ...
  const words = inrToWords(numericTotal); // CORRECTED
